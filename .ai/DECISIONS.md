...
Per-owner coordinators bound each owner to one generation, but N owners would mean N concurrent provider calls, which is not what "bounded generation" was for. A plain counting semaphore around the whole generation — provider calls and the short gate transaction alike — holds the process to one at a time by default, measured with two owners racing and a peak counter. The admission check is a loop rather than an if, because a caller arriving exactly as a waiter wakes could otherwise both take the slot. FIFO, in-process, and nothing more: no job table, no priorities, no router. The bound's number is a working constant.
